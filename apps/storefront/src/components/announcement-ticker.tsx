...
export function AnnouncementTicker() {
  ...
  return (
    <div role="marquee" aria-label="Ankündigungen" className="border-b border-rule bg-raised text-ink-aged">
        ...
        <ul className="flex shrink-0 animate-marquee items-center gap-8 whitespace-nowrap pr-8 text-[0.72rem] font-medium tracking-wide">
          {track.map((p, i) => (
            <li key={i} aria-hidden={i >= points.length ? "true" : undefined} className="flex items-center gap-8">
              <span>{p}</span>
              <span className="h-1 w-1 rounded-full bg-[color:color-mix(in_srgb,var(--w14-ink)_25%,transparent)]" />
            </li>
          ))}
        </ul>
      </div>
