...
import { type SQL, and, asc, count, desc, eq, gte, ilike, inArray, lte, or, sql } from 'drizzle-orm';
...
const productsListRoute: FastifyPluginAsync = async (app) => {
  app.get<{ Querystring: TProductListQuery }>('/api/products', {
    schema: {
      tags: ['products'],
    ...
    const preds: Array<SQL | undefined> = [
      ...
      q.listedOnStorefront !== undefined ? eq(products.listedOnStorefront, q.listedOnStorefront) : undefined,
      ...
      q.archived === true  ? sql`${products.archivedAt} IS NOT NULL` :
      q.archived === false ? sql`${products.archivedAt} IS NULL` : undefined,
