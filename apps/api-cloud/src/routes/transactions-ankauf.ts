...
import {
  AnkaufBody,
  AnkaufResponse,
  type AnkaufBody as TAnkaufBody,
} from '../schemas/ankauf.js';
...
const transactionsAnkaufRoute: FastifyPluginAsync<TransactionsAnkaufOpts> = async (app, opts) => {
  app.post<{ Body: TAnkaufBody }>('/api/transactions/ankauf', {
    schema: {
      tags: ['transactions'],
    ...
    if (
      totalExceedsStepUpThreshold(body.totalEur, opts.env.TRANSACTION_STEP_UP_THRESHOLD_EUR)
    ) {
      requireStepUp(req);
    }
