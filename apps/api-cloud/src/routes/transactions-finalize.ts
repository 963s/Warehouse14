...
import { finalize as finalizeReservation, ReservationOwnershipError } from '@warehouse14/inventory-lock';
import { ledgerEvents, transactionItems, transactionPayments, transactions } from '@warehouse14/db/schema';
...
const transactionsFinalize: FastifyPluginAsync<TransactionsFinalizeOpts> = async (app, opts) => {
  app.post('/api/transactions/finalize', {
    ...
    // ──────────────────────────────────────────────────────────────────
    const existingByKey = (await app.db
      .select({
        id: transactions.id,
    ...
    if (existingByKey) {
      const ledgerRow = (await app.db.select({ id: ledgerEvents.id })
        .from(ledgerEvents)
        .where(drizzleSql`${ledgerEvents.entityTable} = 'transactions' AND ${ledgerEvents.entityId} = ${existingByKey.id}`)
        .limit(1))[0];
    ...
    const outcome = await app.db.transaction(async (tx) => {
      ...
      const ledgerRow = (await tx.select({ id: ledgerEvents.id })
        .from(ledgerEvents)
        .where(drizzleSql`${ledgerEvents.entityTable} = 'transactions' AND ${ledgerEvents.entityId} = ${txRow.id}`)
        .limit(1))[0];
      ...
      if (isUniqueViolation(err, 'transactions_idempotency_key_uniq')) {
        ...
        const ledgerRow = (await app.db.select({ id: ledgerEvents.id })
          .from(ledgerEvents)
          .where(drizzleSql`${ledgerEvents.entityTable} = 'transactions' AND ${ledgerEvents.entityId} = ${winner.id}`)
          .limit(1))[0];
        return {
          id: winner.id,
