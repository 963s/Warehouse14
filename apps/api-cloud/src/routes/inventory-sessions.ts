...
const inventorySessionsRoutes: FastifyPluginAsync = async (app) => {
  app.post('/api/inventory-sessions', {
    ...
    try {
      const result = await app.db.transaction(async (tx) => {
        ...
        `);
        const expected = parseInt(count!.c, 10);
        const [s] = await tx.insert(inventorySessions).values({
  ...
  app.get('/api/inventory-sessions/current', {
    schema: {
      tags: ['inventory'],
  ...
  app.post<{ Params: { id: string }; Body: { rawBarcode: string } }>('/api/inventory-sessions/:id/scans', {
    schema: {
      tags: ['inventory'],
    ...
    const result = await app.db.transaction(async (tx) => {
      const [s] = await tx
        ...
        .from(inventorySessions).where(eq(inventorySessions.id, req.params.id)).limit(1);
      ...
      const [product] = await tx
        ...
        .from(products).where(eq(products.barcode, req.body.rawBarcode)).limit(1);

      let matchStatus: 'MATCHED' | 'UNKNOWN_BARCODE' | 'DUPLICATE' | 'EXPECTED_BUT_SOLD' | 'UNEXPECTED';
      ...
      const [scan] = await tx.insert(inventoryScans).values({
        sessionId: req.params.id,
        rawBarcode: req.body.rawBarcode,
        productId, matchStatus,
  ...
  app.post<{ Params: { id: string }; Body: { notes?: string } }>('/api/inventory-sessions/:id/close', {
    schema: {
      tags: ['inventory'],
      summary: 'Close session: compute Schwund + matched + unexpected.',
      params: Type.Object({ id: Type.String({ format: 'uuid' }) }),
      body: Type.Object({ notes: Type.Optional(Type.String({ maxLength: 2048 })) }),
      response: { 200: SessionView, 401: ErrorResponse, 403: ErrorResponse, 404: ErrorResponse, 409: ErrorResponse },
    },
  }, async (req, reply) => {
    requireAuth(req);
    requireRole(req, 'ADMIN');
    ...
    const result = await app.db.transaction(async (tx) => {
      const [s] = await tx
        .select().from(inventorySessions)
        .where(eq(inventorySessions.id, req.params.id)).limit(1);
      if (!s) throw new SessionNotFoundError(`Inventory session ${req.params.id} not found.`);
      if (s.status !== 'OPEN') throw new SessionConflictError('Session is already CLOSED.');

      const [counts] = await tx.execute<{ matched: string; unexpected: string; missing: string }>(drizzleSql`
      ...
      const matched = parseInt(counts!.matched, 10);
      const missing = parseInt(counts!.missing, 10);
      const unexpected = parseInt(counts!.unexpected, 10);

      const [updated] = await tx
        .update(inventorySessions).set({
          status: 'CLOSED',
          closedAt: new Date(),
        ...
        }).where(eq(inventorySessions.id, s.id)).returning();
