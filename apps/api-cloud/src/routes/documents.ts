...
const documentsRoutes: FastifyPluginAsync = async (app) => {
  ...
  // POST /api/documents
  // ────────────────────────────────────────────────────────────────────
  app.post<{ Body: TCreateDocumentBody }>('/api/documents', {
    schema: {
      tags: ['documents'],
  ...
  // ────────────────────────────────────────────────────────────────────
  app.get<{ Querystring: TListDocumentsQuery }>('/api/documents', {
    schema: {
      tags: ['documents'],
    ...
    const preds: Array<SQL | undefined> = [
      ...
      q.transactionId !== undefined ? eq(documentAttachments.transactionId, q.transactionId) : undefined,
      q.appraisalId !== undefined ? eq(documentAttachments.appraisalId, q.appraisalId) : undefined,
  ...
  app.get<{ Params: SugarParams }>('/api/customers/:id/documents', {
    schema: {
  ...
  app.get<{ Params: SugarParams }>('/api/products/:id/documents', {
    schema: {
  ...
  app.get<{ Params: SugarParams }>('/api/transactions/:id/documents', {
    schema: {
  ...
  app.get<{ Params: SugarParams }>('/api/appraisals/:id/documents', {
    schema: {
  ...
  // ────────────────────────────────────────────────────────────────────
  app.post<{ Params: TDocumentIdParams }>('/api/documents/:id/archive', {
    schema: {
      tags: ['documents'],
    ...
    const result = await app.db.transaction(async (tx) => {
      ...
      if (!row) throw new DocumentNotFoundError(`Document ${req.params.id} not found or already archived`);
