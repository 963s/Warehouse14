...
const customersRoutes: FastifyPluginAsync = async (app) => {
  ...
  // POST /api/customers
  // ══════════════════════════════════════════════════════════════════════
  app.post<{ Body: TCreateCustomerBody }>('/api/customers', {
    schema: {
      tags: ['customers'],
  ...
  // ══════════════════════════════════════════════════════════════════════
  app.get<{ Params: { id: string } }>('/api/customers/:id', {
    schema: {
      tags: ['customers'],
    ...
    return reply.status(200).send({
      ...
      kycStatus: row.kyc_status as 'NOT_REQUIRED' | 'PENDING' | 'COMPLETED' | 'EXPIRED' | 'FAILED',
  ...
  // ══════════════════════════════════════════════════════════════════════
  app.get<{ Params: { id: string } }>('/api/customers/:id/products', {
    schema: {
      tags: ['customers'],
  ...
  // ══════════════════════════════════════════════════════════════════════
  app.get<{ Params: { id: string } }>('/api/customers/:id/transactions', {
    schema: {
      tags: ['customers'],
