...
const tasksRoutes: FastifyPluginAsync = async (app) => {
  ...
  // POST /api/tasks
  // ────────────────────────────────────────────────────────────────────
  app.post<{ Body: TCreateTaskBody }>('/api/tasks', {
    schema: {
      tags: ['tasks'],
  ...
  // GET /api/tasks
  // ────────────────────────────────────────────────────────────────────
  app.get<{ Querystring: TListTasksQuery }>('/api/tasks', {
    schema: {
      tags: ['tasks'],
    ...
    const preds: Array<SQL | undefined> = [
      ...
      assigneeFilter !== undefined ? eq(internalTasks.assignedToUserId, assigneeFilter) : undefined,
    ...
    const [rows, totalRow] = await Promise.all([
      app.db
        ...
        .from(internalTasks)
        .where(whereClause)
        .orderBy(desc(internalTasks.priority), asc(internalTasks.dueDate), desc(internalTasks.createdAt))
        .limit(limit)
        .offset(offset),
  ...
  // GET /api/tasks/:id
  // ────────────────────────────────────────────────────────────────────
  app.get<{ Params: TTaskIdParams }>('/api/tasks/:id', {
    schema: {
      tags: ['tasks'],
      summary: 'Fetch a single task.',
      params: TaskIdParams,
      response: { 200: GetTaskResponse, 404: ErrorResponse, 401: ErrorResponse, 403: ErrorResponse },
    },
  }, async (req, reply) => {
    requireAuth(req);
    requireRole(req, 'ADMIN', 'CASHIER');
  ...
  // ────────────────────────────────────────────────────────────────────
  app.patch<{ Params: TTaskIdParams; Body: TUpdateTaskBody }>('/api/tasks/:id', {
    schema: {
      tags: ['tasks'],
  ...
  app.patch<{ Params: TTaskIdParams; Body: TTransitionTaskStatusBody }>(
    ...
    async (req, reply) => {
      ...
      if (next === 'CANCELLED' && (!req.body.cancellationReason || req.body.cancellationReason.length < 4)) {
        throw new TaskValidationError('cancellationReason (≥ 4 chars) required when status=CANCELLED');
      }

      // Read current state inside a transaction so the transition check is atomic.
      const result = await app.db.transaction(async (tx) => {
        const [current] = await tx
          .select({ id: internalTasks.id, status: internalTasks.status, startedAt: internalTasks.startedAt })
          .from(internalTasks)
          .where(eq(internalTasks.id, req.params.id))
          .limit(1);
