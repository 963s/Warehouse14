/**
 * eBay-Kanal — das geteilte deutsche Vokabular (Phasen, Übergangs-Verben,
 * Konflikt-Texte, Quellen-Labels, Veröffentlichungs-Auslegung).
 *
 * Die Wahrheit der Zustandsmaschine lebt im api-client
 * (`ALLOWED_EBAY_TRANSITIONS`, `EBAY_STATE_ORDER`, `EBAY_STATE_LABELS`) und im
 * Server-Trigger. Dieses Modul ÜBERSETZT diese Wahrheit nur. Es erfindet
 * nichts: jeder Übergang wird gegen `ALLOWED_EBAY_TRANSITIONS` geprüft, jede
 * Konflikt-Zeile gegen den echten `inventorySideEffect`.
 *
 * Framework-frei (kein React, kein React-Native), damit die Telefon-App UND
 * die Kasse dieselben Sätze sprechen. Gerätespezifisches (Badge-Varianten,
 * Farben) bleibt in der jeweiligen App.
 */
import {
...
export interface EbayTransitionOption {
  ...
  hint: string
  /**
   * Ob der Schritt ein Zurücksetzen ist (z. B. ONLINE zurück zu ENTWURF).
   * Solche Schritte rendert die UI als sekundäre Aktion, nicht als Haupt-Knopf.
   */
  isRevert: boolean
}

/** Statische Beschreibung je Zielzustand (Verb + Hinweis), unabhängig vom Start. */
const TRANSITION_META: Readonly<Record<EbayState, { actionLabel: string; hint: string }>> = {
  ENTWURF: {
  ...
  VERKAUFT: {
    ...
    hint: "Der Käufer steht fest. Der Bestand wird reserviert.",
  },
  BEZAHLT: {
 ...
 * nie eingebucht, also nur ENTWURF). Liest direkt aus ALLOWED_EBAY_TRANSITIONS,
 * so dass die UI nie einen Knopf zeigt, den der Server mit 409 ablehnen würde.
 */
...
// passiert ist. Wir übersetzen jeden Fall in eine ehrliche deutsche Zeile und
// einen Schweregrad. Ein Konflikt bedeutet: derselbe Artikel ist hier im Laden
...
export interface SideEffectMeta {
  /** Ob es überhaupt eine Meldung wert ist (NONE zeigt nichts). */
  show: boolean
  ...
  switch (effect) {
    ...
    case "IDEMPOTENT_NO_OP":
      return {
        ...
        message: "Der Artikel war bereits für eBay reserviert. Der Bestand bleibt unverändert.",
      }
...
// Der publish-Endpunkt liefert `configured=false`, wenn kein eBay-Zugang
...
export interface PublishMeta {
   ...
   * und sicher anzuzeigen), fällt sonst auf einen festen Text zurück.
   */
  message: string
...
export function describePublish(res: EbayPublishResponse): PublishMeta {
  if (res.published) {
    return {
      ...
      message: res.detail || "Die Listung ist live. Der Zustand wurde auf Online gesetzt.",
      isLive: true,
    }
  }
  if (!res.configured) {
  ...
  // Konfiguriert, aber nicht live: der Server hat aus einem fachlichen Grund
  // nicht veröffentlicht (selten; der detail-Text erklärt es).
  return {
...
// Spur ehrlich: ein Owner-Schritt vom Telefon, ein eBay-Webhook, der Abgleich-
...
// Aus einer Liste von Zuständen die Anzahl je Phase und das Gesamt. Reine
